...
import { 
  ...
  FiThumbsUp, FiThumbsDown, FiLoader
} from 'react-icons/fi'
...
const BlogPost = () => {
  ...
  const renderContent = (content) => {
    ...
    lines.forEach((line, index) => {
      // Headers
      if (line.startsWith('# ')) {
        if (inList) { elements.push(<ul key={`list-end-${index}`} className="mb-4"></ul>); inList = false }
        elements.push(<h1 key={index} className="text-3xl font-bold text-gray-900 mt-8 mb-4">{line.slice(2)}</h1>)
      }
      else if (line.startsWith('## ')) {
        if (inList) { elements.push(<ul key={`list-end-${index}`} className="mb-4"></ul>); inList = false }
        elements.push(<h2 key={index} className="text-2xl font-bold text-gray-900 mt-6 mb-3">{line.slice(3)}</h2>)
      }
      else if (line.startsWith('### ')) {
        if (inList) { elements.push(<ul key={`list-end-${index}`} className="mb-4"></ul>); inList = false }
        elements.push(<h3 key={index} className="text-xl font-semibold text-gray-900 mt-5 mb-2">{line.slice(4)}</h3>)
      }
      // Bullet points
      else if (line.startsWith('- ') || line.startsWith('* ')) {
        if (!inList) { 
          elements.push(<ul key={`list-start-${index}`} className="list-disc ml-6 mb-4 space-y-1"></ul>)
          inList = true 
        }
        // Add to the last ul element
        const lastUl = elements[elements.length - 1]
        if (lastUl && lastUl.type === 'ul') {
          // We need to handle this differently since we can't directly append to React elements
          elements.push(<li key={index} className="text-gray-700 leading-relaxed">{line.slice(2)}</li>)
        }
      }
      // Numbered list
      else if (/^\d+\.\s/.test(line)) {
        if (!inList) { 
          elements.push(<ul key={`numlist-start-${index}`} className="list-decimal ml-6 mb-4 space-y-1"></ul>)
          inList = true 
        }
        const content = line.replace(/^\d+\.\s/, '')
        elements.push(<li key={index} className="text-gray-700 leading-relaxed">{content}</li>)
      }
      // Empty line
      else if (line.trim() === '') {
        if (inList) { 
          elements.push(<div key={`list-end-${index}`} className="h-2"></div>)
          inList = false 
        } else {
      ...
      // Paragraph
        ...
        if (inList) { elements.push(<ul key={`list-end-${index}`} className="mb-4"></ul>); inList = false }
        elements.push(<p key={index} className="text-gray-700 leading-relaxed mb-4">{line}</p>)
      }
    })
    ...
    return (
      <div className="min-h-[60vh] flex items-center justify-center">
        <div className="text-center">
          <FiLoader className="animate-spin text-4xl text-green-600 mx-auto" />
            ...
            <div className="flex flex-wrap items-center gap-4 mt-6 text-sm text-gray-500">
              <div className="flex items-center gap-2">
                <div className="w-10 h-10 rounded-full bg-green-100 flex items-center justify-center text-green-700 font-semibold">
                  {blog.author?.firstName?.[0] || 'A'}{blog.author?.lastName?.[0] || 'U'}
                </div>
                <div>
            ...
            >
              ...
              {blog.tags && blog.tags.length > 0 && (
                <div className="flex flex-wrap gap-2 mt-8 pt-6 border-t border-gray-100">
                  <span className="text-sm text-gray-500 mr-2 font-medium">Tags:</span>
